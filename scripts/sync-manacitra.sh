...
async function run() {
  ...
  const checks = [
    ...
    { id: 'sampada',     cmd: "ssh -o StrictHostKeyChecking=accept-new 140.245.227.176 'curl -s -o /dev/null -w %{http_code} http://localhost:3002 2>/dev/null' 2>/dev/null | grep -qE '^[234]'", ssh: true },
    ...
    { id: 'sampada',     url: 'https://sampada.pages.dev' },
